...
$file = fopen('eurostat_monthly_asylum.csv', 'w');	

/*	Set the column headers for	csv filename */ 
fputcsv($file, array('Index Num', 'Country of Origin','Country of Origin-ISO2', 'Monthly Asylum Data', 'Year', 'Month', 'Age', 'Sex', 'Country of Asylum'));
...
{	
	$url = "http://ec.europa.eu/eurostat/wdds/rest/data/v2.1/json/en/migr_asyappctzm?citizen=".$origin_country2[$a]."&precision=1&asyl_app=NASY_APP&time=2019M09&time=2019M10&time=2019M11&time=2019M12";

	echo $url."</br>"; 

	$result = json_decode(file_get_contents($url));
	$monthly_asylum_data =	(array) $result->value;
	...
	$monthly_asylum_data2 = sizeof($geo_array) * sizeof($time_array) * sizeof($sex_array) * sizeof($age_array);
	...
	for ($l=0; $l < $monthly_asylum_data2; $l++)	
		{
			/*	check wather the value from asylum array is numeric or not)  */
			if (isset($monthly_asylum_data[$l]))
			{
				$asylum_value = $monthly_asylum_data[$l];
			}
			elseif(empty($monthly_asylum_data[$l]) )
			{
				$asylum_value = 0;	
			}
			else
			...
			fputcsv($file, array($index, $origin_country_name[$a], $origin_country2[$a], $asylum_value, $year, $month, $age_array[$age_flag], $sex_array[$sex_flag], $geo_array[$geo_flag]));
			$time_flag++;
